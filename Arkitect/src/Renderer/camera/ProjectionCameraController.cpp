...
namespace Arkitect {
	...
	ProjectionCameraController::ProjectionCameraController(float aspectRatio, float fov)
		:m_Camera(ProjectionCamera(fov,aspectRatio, 0.1, 1000.0))
	{
	}
	...
	void ProjectionCameraController::OnUpdate(Deltatime dt)
	{
		...
		{
			m_CameraPosition += m_Camera.GetFrontDirection() * m_CameraMovementSpeed * dt.GetSeconds();
		}
		...
		{
			m_CameraPosition -= glm::normalize(glm::cross(m_Camera.GetFrontDirection(), glm::vec3(0.0, 1.0, 0.0))) * m_CameraMovementSpeed * dt.GetSeconds();
		}
		...
		{
			m_CameraPosition -= glm::vec3(0.0, 1.0, 0.0) * m_CameraMovementSpeed * dt.GetSeconds();
		}
		...
		if (m_MousePressed) {
			...
			m_Camera.SetHorizontalAngle(m_CameraHorizontalAngle + m_TempHAngle);
			m_Camera.SetVerticalAngle(m_CameraVerticalAngle + m_TempVAngle);
		}
	}
	...
	bool ProjectionCameraController::OnMouseButtonReleasedEvent(MouseButtonReleasedEvent& e)
	{
		if (e.GetMouseButton() == Mouse::Secondary) {
			...
			m_CameraHorizontalAngle += m_TempHAngle;
			m_CameraVerticalAngle += m_TempVAngle;
